    ...
    switch($key){
       ...
       case "Insert-records-to-db":
			...
			if($mop == "Cash"){
				...
				$remarks = "Donation/Unsecured Loan/Other From ".$name."(".$person_id.") by ".$mop;
				...
				$remarks = "Donation/Unsecured Loan/Other From ".$name."(".$person_id.") by ".$mop."(".$pno.")";
	   ...
	   case "Load-alla-bills":
			$sql = "SELECT *, donations.id AS rec_id FROM donations JOIN personal_info ON (personal_info.id = donations.person_id) WHERE donations.amount != 0";
			$result = $db->query($sql); ?>
			<tr class="tab-head">
				<td>Reciept No.</td>
				<td>Date</td>
...
//fetch single Bill details and add functionaity for updation
	   case "fetch-single-bill-details":
			$id = $_REQUEST['id'];
			$sql = "SELECT *, donations.id AS rec_id FROM donations JOIN personal_info ON (personal_info.id = donations.person_id) WHERE donations.amount != 0 AND donations.id='$id'";
			$result = $db->query($sql);
			if(count($result)>0){
				while($row=$result->fetchArray(SQLITE3_ASSOC)){
					$pid = $row['person_id'];
					...
					<div class="prev prev-copy">
						...
						<div class="blanks b5">
							<div>on Account Membership Fee</div> <strong class="mem-fee "><?php echo "<i class='fa fa-inr'></i>".number_format($mem_fee)."/-"; ?></strong> <strong readonly class="s2 mem-fee-words umem-fee-words"></strong><div class="d2">&</div>
						</div>
						
						<div class="blanks b6">
							<div>Voluntarily Donated/Unsecured Loan/Other</div> <strong class="amount s2"><?php echo "<i class='fa fa-inr'></i>".number_format(abs($amount))."/-"; ?></strong> <strong class="amt-words">  </strong>
						</div>	

						<div class="blanks b7">
							&nbsp;<div>Total Amount</div> <strong class="total"> <?php echo "<i class='fa fa-inr'></i>".number_format($amount+$mem_fee)."/-"; ?></strong> <div class="d2"> By </div> <div class="mop"><?php echo $mop; ?></div> &nbsp; <input type="text" readonly value="<?php echo $pno; ?>" class="pno upno"> <div class="d3">Dated</div> <input type="text" readonly value="<?php echo $pdate; ?>" class="date udate s2">
						</div>
...
//fetch single Bill details and add functionaity for updation ends here//
	   ...
	   break;
			...
			if(numr($result)>0){
	   			while ($row=$result->fetchArray(SQLITE3_ASSOC)) {
	   				...
	   				if(!empty($mobile) AND !empty($msg)){
						...
						if(strpos( $resultpro, "Message Succesfully Sent") == false ) echo "<div class='alert alert-danger'>".$resultpro."</div>";
					} ?> <script type="text/javascript">$(".loading-gif").hide();</script><?php
	   			} ?>
		...
		case 'get-contacts-suggestions-to-send-sms':
			$value = $_REQUEST['val'];
			$sql = "SELECT mobile, name FROM personal_info WHERE mobile LIKE '%$value%' OR name LIKE '%$value%'";
			$result = $db->query($sql);
			if(numr($result)>0){
	   			while ($row=$result->fetchArray(SQLITE3_ASSOC)) {
	   				$mobile = $row['mobile'];
		...
		case "change-password": ?>
			<input type="text" class="new-pass form-control" placeholder="New Password">
			<script type="text/javascript">
				$(".new-pass").change(function(){
					$.post("ajax-req-handler.php", { key: "save-new-password", pass: $(this).val() }, function(data){ $.alert(data); });
				});
			</script> <?php 
		break;
		...
		case "save-new-password":
			$new_pass = $_POST['pass'];
			$sql = "UPDATE settings SET value='$new_pass' WHERE id='2' AND setting='password' ";
			if($db->query($sql)){ ?>
				<div class="alert alert-success">Password Successfully Changed</div> <?php
			}
			else{ ?>
				<div class="alert alert-danger">Error! <?php echo $db->lastErrorMsg(); ?></div> <?php
			}
		break;
    }
    $db->close();
